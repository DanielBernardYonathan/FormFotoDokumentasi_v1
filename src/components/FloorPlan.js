...
const FloorPlan = ({ formData, onPhotoComplete, onBackToForm, photos }) => {
  ...
  const photoPointsByPage = {
    1: [
      ...
      { id: 7, x: 67.8, y: 86, label: "KANAN BAHU JALAN" },
      { id: 8, x: 66, y: 82.5, label: "TAMPAK KANAN DEPAN KEBELAKANG" },
      { id: 9, x: 33.5, y: 81.8, label: "TAMPAK KIRI DEPAN KEBELAKANG" },
      { id: 10, x: 41.4, y: 60.2, label: "KIRI TERAS LUAR" },
      { id: 11, x: 59, y: 60, label: "KANAN TERAS LUAR" },
      { id: 12, x: 61.5, y: 56.5, label: "KANAN TERAS DALAM" },
      { id: 13, x: 39, y: 56.5, label: "KIRI TERAS DALAM" },
      { id: 14, x: 58.8, y: 37.5, label: "SUDUT KANAN DEPAN AREA SALES" },
      { id: 15, x: 39, y: 52, label: "SUDUT KIRI DEPAN AREA SALES" },
      { id: 16, x: 39, y: 28.2, label: "SUDUT KIRI BELAKANG AREA SALES" },
      { id: 17, x: 61.5, y: 27.5, label: "SUDUT KANAN BELAKANG AREA SALES" },
      { id: 18, x: 61.7, y: 22.2, label: "BELAKANG + JANITOR" },
      { id: 19, x: 59.5, y: 12.5, label: "KAMAR MANDI" },
      { id: 20, x: 53.1, y: 16.2, label: "GUDANG SEBELAH KANAN" },
      { id: 21, x: 38.6, y: 13, label: "GUDANG SEBELAH KIRI" },
      { id: 22, x: 65.1, y: 11.3, label: "KANAN BELAKANG BANGUNAN" },
      { id: 23, x: 63.7, y: 7.5, label: "KANAN BELAKANG BANGUNAN MENGHADAP SAMPING" },
      { id: 24, x: 37.5, y: 7.5, label: "KIRI BELAKANG BANGUNAN MENGHADAP SAMPING" },
      { id: 25, x: 35, y: 11, label: "KIRI BELAKANG BANGUNAN MENGHADAP DEPAN" },
      { id: 26, x: 57.6, y: 63.8, label: "KOLOM IWF DUDUKAN LISTPLANK" },
      { id: 30, x: 58.2, y: 81.7, label: "INSTALASI LISTRIK POLE SIGN" },
      { id: 32, x: 61.1, y: 51, label: "INSTALASI LISTRIK MEJA KASIR" },
      { id: 33, x: 48.5, y: 23.5, label: "INSTALASI LISTRIK & DRAINASE CHILLER" },
      { id: 34, x: 42.4, y: 45.5, label: "INSTALASI LISTRIK FREEZER" },
      { id: 35, x: 56.8, y: 73.3, label: "GUTTER" },
      { id: 36, x: 48.7, y: 49.4, label: "PINTU KACA ALLUMUNIUM" },
  ...
  const allPhotoPoints = [...photoPointsByPage[1], ...photoPointsByPage[2], ...photoPointsByPage[3]]
  const currentPagePoints = photoPointsByPage[currentPage] || []

  const getFloorPlanImage = () => {
    switch (currentPage) {
      case 1: return "/floor.png"
      case 2: return "/floor2.jpeg"
      case 3: return "/floor3.jpeg"
      default: return "/floor.png"
    }
  }

  const handlePointClick = (point) => {
    if (point.id !== currentPhotoNumber) {
      alert(`Harap ambil foto secara berurutan. Foto nomor ${currentPhotoNumber} harus diambil terlebih dahulu.`)
      return
    }
  ...
  const handlePhotoCapture = (photoData) => {
    const newPhotos = {
      ...capturedPhotos,
      [selectedPoint.id]: { ...photoData, point: selectedPoint, timestamp: new Date().toISOString() },
    }
  ...
  const handleCameraClose = () => { setShowCamera(false); setSelectedPoint(null) }
  ...
  const generatePDFReport = async () => {
    ...
    const drawHeader = () => {
      ...
      pdf.text("FOTO DOKUMENTASI BANGUNAN TOKO BARU", pageWidth / 2, 18, { align: "center" })
    }
    const drawFooter = (pageNumber) => {
      pdf.setFontSize(8)
      pdf.setTextColor(150, 150, 150)
      pdf.text(`Halaman ${pageNumber}`, pageWidth - margin, pageHeight - 10, { align: "right" })
    }
    ...
    const photoWidth = (pageWidth - 2 * margin - (photosPerRow - 1) * 4) / photosPerRow
    ...
    for (const point of sortedPhotoPoints) {
      ...
      if (photo && photo.blob) {
        ...
        pdf.addImage(imageData, "JPEG", startX + 1, yPos + 1, photoWidth - 2, photoHeight - 2)
      } else {
        pdf.setFontSize(8)
      ...
      pdf.setFont("helvetica", "normal")
      pdf.setTextColor(0, 0, 0)
      const labelText = pdf.splitTextToSize(point.label, photoWidth - pdf.getTextWidth(idText))
  ...
  async function savePdfToSpreadsheet() {
    try {
      ...
      const payload = {
        nomorUlok: formData.nomorUlok || "",
        namaToko: formData.namaToko || "",
        kodeToko: formData.kodeToko || "",
        cabang: formData.cabang || "",
        tanggalGo: formData.tanggalGo || "",
        tanggalSt: formData.tanggalSt || "",
        tanggalAmbilFoto: formData.tanggalAmbilFoto || "",
        spkAwal: formData.spkAwal || "",
        spkAkhir: formData.spkAkhir || "",
        kontraktorSipil: formData.kontraktorSipil || "",
        kontraktorMe: formData.kontraktorMe || "",
        pdfBase64,
      }

      const res = await fetch("http://127.0.0.1:8000/save-toko", {
        method: "POST",
  ...
  return (
    <div className="floor-plan-container">
      ...
      <div className="floor-plan-header">
        <button onClick={onBackToForm} className="btn-back">← Kembali ke Form</button>
        <div className="store-info">
          <h2>{formData.namaToko} - {formData.kodeToko}</h2>
      ...
      <div className="floor-plan-content">
        <div className="floor-plan-wrapper">
          <div className="floor-plan-image">
            <img src={getFloorPlanImage()} alt={`Floor Plan Halaman ${currentPage}`} className="floor-plan-bg" />
            {currentPagePoints.map((point) => (
              <button key={point.id} className={`photo-point ${getPointStatus(point.id)}`}
                style={{ left: `${point.x}%`, top: `${point.y}%`, position: "absolute", transform: "translate(-50%, -50%)" }}
                onClick={() => handlePointClick(point)}
                disabled={point.id !== currentPhotoNumber && !capturedPhotos[point.id]}
                title={`${point.id}. ${point.label}`}>
                {point.id}
                {capturedPhotos[point.id] && <span className="check-mark">✓</span>}
              </button>
            ))}
          </div>
          ...
          <div className="photo-list">
            <div className="photo-grid">
              {allPhotoPoints.map((point) => (
                <div key={point.id} className={`photo-item ${getPointStatus(point.id)}`}>
                  <div className="photo-number">{point.id}</div>
                  <div className="photo-label">{point.label}</div>
                  {capturedPhotos[point.id] && (
                    <div className="photo-preview">
                      <img src={capturedPhotos[point.id].url} alt={`Foto ${point.id}`} className="thumbnail" />
                    </div>
                  )}
                </div>
              ))}
      ...
      {showCamera && selectedPoint && (
        <CameraCapture point={selectedPoint} onCapture={handlePhotoCapture} onClose={handleCameraClose} />
      )}
      ...
      {completedCount === 38 && (
        <div className="completion-modal">
          <div className="modal-content">
            ...
            <button className="btn-primary" onClick={savePdfToSpreadsheet} disabled={isSaving}>
      ...
      {isSaving && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2000}}>
          <div style={{background:"#fff",padding:"16px 20px",borderRadius:8,fontWeight:600}}>Menyimpan...</div>
        </div>
      )}
      {toast && (
        <div style={{position:"fixed",top:16,right:16,zIndex:2001,background:toast.type==="success"?"#16a34a":"#dc2626",color:"#fff",padding:"10px 14px",borderRadius:8,fontWeight:600}}>
          {toast.text}
        </div>
      )}
    </div>
